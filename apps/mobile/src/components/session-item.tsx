    ...
    <View className="rounded-xl border border-[#d8dfdb] bg-white p-4">
      <View className="mb-3 flex-row items-center justify-between">
        <View className="rounded-full border border-[#d8dfdb] bg-[#f8fbf9] px-3 py-1">
          <Text className="text-xs uppercase tracking-[1.2px] text-[#4f6159]">{t("session.label")}</Text>
        </View>
        <View className="flex-row items-center gap-1 rounded-full border border-[#d8dfdb] bg-[#f8fbf9] px-3 py-1">
          ...
          <Text className="text-sm text-[#173129]">{formatMinutes(sessionMinutes(session), language)}</Text>
        </View>
      </View>

      <Text className="text-base text-[#15241f]">{formatDayLabel(session.startAt, language)}</Text>
      <Text className="mt-1 text-sm text-[#5d6963]">{formatSessionRange(session, language)}</Text>
