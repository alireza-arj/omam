...
- `apps/api`: Elysia API with Prisma + PostgreSQL
...
1. Install dependencies at the repo root with your package manager.
2. In `apps/api`, copy `.env.example` to `.env` if needed.
3. Run `npm run db:generate` and `npm run db:push` at the repo root.
4. Start the API with `npm run dev:api`.
5. In `apps/mobile`, copy `.env.example` to `.env`.
6. Start the app with `npm run dev:mobile` and open it in Expo Go.
...
- After changing `.env`, restart Expo (`npm run dev:mobile`) so env values refresh.
